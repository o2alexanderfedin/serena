...
def test_write_manifest_lands_at_marketplace_json(tmp_path: Path) -> None:
    """§ 3.1: write_manifest must write to .claude-plugin/marketplace.json, not root."""
    _make_plugin_tree(
        tmp_path,
        dir_name="o2-scalpel-rust",
        plugin_name="o2-scalpel-rust",
    ...
    assert out == tmp_path / ".claude-plugin" / "marketplace.json"
    assert out.is_file()
...
def test_write_manifest_creates_claude_plugin_subdir(tmp_path: Path) -> None:
    """§ 3.1: write_manifest must create .claude-plugin/ if absent."""
    m = build_manifest(tmp_path, generator_sha="test")
    assert not (tmp_path / ".claude-plugin").exists()
    write_manifest(tmp_path, m)
    assert (tmp_path / ".claude-plugin" / "marketplace.json").is_file()


def test_write_manifest_does_not_write_to_repo_root(tmp_path: Path) -> None:
    """§ 3.1: marketplace.json must NOT appear at the repo root."""
    m = build_manifest(tmp_path, generator_sha="test")
    write_manifest(tmp_path, m)
