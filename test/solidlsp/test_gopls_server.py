"""Stream 6 / Leaf B — GoplsServer adapter unit tests.

These tests are *unit* level: they exercise the adapter's constants and
small introspection methods without spawning a real gopls subprocess.
The end-to-end smoke (boot + initialize + documentSymbol round-trip)
lives in ``test/solidlsp/go/`` once gopls is confirmed on the host
(the existing Go integration tests cover that path).

The adapter mirrors the ``VtslsServer`` shape
...
def test_server_id_constant_is_gopls() -> None:
    """``server_id`` keys dynamic-capability registrations; must be stable."""
    assert GoplsServer.server_id == "gopls"


def test_get_language_enum_instance_returns_go() -> None:
    """Identity tie-back to ``Language.GO`` (per Stage 1E pattern)."""
    assert GoplsServer.get_language_enum_instance() == Language.GO


def test_is_ignored_dirname_includes_vendor_and_testdata(tmp_path) -> None:
    """vendor / testdata / dist dominate Go project trees."""
    if shutil.which("gopls") is None:
        pytest.skip("gopls not on PATH; skipping adapter instantiation")
    cfg = LanguageServerConfig(code_language=Language.GO)
    srv = GoplsServer(cfg, str(tmp_path), SolidLSPSettings())
    assert srv.is_ignored_dirname("vendor") is True
    assert srv.is_ignored_dirname("testdata") is True
...
def test_initialize_params_advertise_workspace_apply_edit(tmp_path) -> None:
    ...
    params = cast(dict[str, Any], GoplsServer._get_initialize_params(str(tmp_path)))
...
def test_initialize_params_advertise_code_action_kinds(tmp_path) -> None:
    """gopls code action kinds must include the Go refactor family."""
    params = cast(dict[str, Any], GoplsServer._get_initialize_params(str(tmp_path)))
...
def test_initialize_params_language_id_is_go(tmp_path) -> None:
    """``language_id`` must be ``"go"`` to match gopls expectations."""
